...
#passando pouco contexto apra a cohere - ementa limpa
    ...
    try:
        ...
        # ---------------------------------------------------------
        # NOVO: EXPANSÃO DE CONSULTA (QUERY EXPANSION)
        # Ensinamos a Cohere a valorizar o contexto jurídico para não
        # punir projetos importantes (ex: Marco Regulatório, normas).
        # ---------------------------------------------------------
        
        query_enriquecida = f"Projeto de lei, regulamentação, normas ou proposição legislativa sobre: {query}"
...
def processar_lote(dados, pkl_data, query_embedding, query_embedding_secundaria, termos_usuario, model, sufixo_leg):
    ...
    for idx, score_tensor in enumerate(cos_scores_principal):
        ...
        # ---------------------------------------------------------
        # NOVO: APLICAÇÃO DO CORTE DINÂMICO
        # ---------------------------------------------------------
...
# O código segue para salvar 'resultados_finais' normalmente
